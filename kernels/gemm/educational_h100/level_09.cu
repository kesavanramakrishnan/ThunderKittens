    ...
    if (warpgroupid == 0) {
        ...
        if (warpgroup::laneid() == 0) {
            ...
            for (int i = 0; i < num_tiles; i++, q_idx++) {
                ...
                tma::load_async(As[q_idx], g.A, {0, 0, row, i}, full[q_idx]);
                tma::load_async(Bs[q_idx], g.B, {0, 0, i, col}, full[q_idx]);
            }
        }
    } else {
        warpgroup::increase_registers<256>();
        rt_fl<16,BLOCK_SIZE> C_accum;
        kittens::warp::zero(C_accum); //zero out accumulator
        if (warpgroup::laneid() == 0) {
            for (int i = 0; i < QSIZE; i++) {
                arrive(empty[i], 1);
            }
        ...
        for (int i = 0; i < num_tiles; i++, q_idx++) {
            ...
            wait(full[q_idx], p);
            warpgroup::mma_AB(C_accum, As[q_idx], Bs[q_idx]);
